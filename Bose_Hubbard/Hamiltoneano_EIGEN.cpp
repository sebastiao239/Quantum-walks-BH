...
int main(){
	...
	///////////////////////////////////////////////////////////
	/////////////APARTIR DE AQUI SE USA EIGEN  ////////////////
	///////////////////////////////////////////////////////////
	Eigen::VectorXd eigvals2(D_exted);
	Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigensolver(H); //Se obtienen los eigen
	Eigen::VectorXd eigvals = eigensolver.eigenvalues();// Aquí se colocan los eigenvalores
	Eigen::MatrixXd Change_E_F = eigensolver.eigenvectors(); // Aqui se colocan los eigenvectores (columnas)
	Eigen::MatrixXd Change_F_E=Change_E_F.transpose(); // MAtriz cambio de base de Fock a Energias
	Eigen::VectorXcd Vec0_F(D); // Vector inicial en base de Fock
	Vec0_F(0)=1.0;
	Eigen::VectorXcd Vec0_E=Change_F_E*Vec0_F; // Vector inicial en base de Energias
	Eigen::VectorXcd Evol_temporal_E(D); //Evolución temporal
	Eigen::VectorXcd Edo_F_t(D); //DEclaramos el estado al tiempo t en la base de energias
	Eigen::MatrixXcd Rho_C(D,D); // Declaramos la matriz de densidad
	Eigen::MatrixXcd Rho_C_reducida(D_exted,D_exted);
	Eigen::MatrixXcd Rho_C_trans_parcial(D_exted,D_exted);
	int bra;
	int ket;
	int NOSE[M];
	int base[M];
	...
	int base_ex2[2];	
	double Negativity;
	ofstream Matrix ("Negativity_EIGEN.dat"); //Al archivo que va a salir
	
	
	
	int Prueba=0;
	Eigen::VectorXd Resultados(D);
	for(int i=0; i<D; i++){
	...
	while(t<Time_f){//t=0.0; // Tiempo
		////////EVOLUCION TEMPORAL DEL ESTADO INICIAL////////////
		Evol_temporal_E=eigvals*t*(-1i); //Operador Evolución temporal
		Evol_temporal_E=Evol_temporal_E.array().exp(); 
		for(int i=0; i<D; i++){ // Evolución temporal
			Edo_F_t(i)=Vec0_E(i)*Evol_temporal_E(i);
		}
		Edo_F_t=Change_E_F*Edo_F_t; //El cambio de base para el estado final al tiempo t en la base de Fock
		/////////////////////////////////////////////////////////
		
		
		//////MATRIZ DE DENSIDAD////////////////////////////////
		Rho_C=Edo_F_t*Edo_F_t.adjoint();
		//cout<<Rho_C.trace()<<endl;
		///////////////////////////////////////////////////////
		...
		Eigen::SelfAdjointEigenSolver<Eigen::MatrixXcd> eigensolver2(Rho_C_trans_parcial); //Se obtienen los eigen
		
		eigvals2=eigensolver2.eigenvalues();
		//cout<<eigvals2<<endl;
